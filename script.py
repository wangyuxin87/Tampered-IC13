...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    # print('matchedSum:{}'.format(matchedSum))
    # print('numGlobalCareGt:{}'.format(numGlobalCareGt))
    # print('numGlobalCareDet:{}'.format(numGlobalCareDet))
...
if __name__=='__main__':
    eval_true = {}
    eval_fake = {}
    input = dict([s[1:].split('=') for s in sys.argv[1:]])
    if len(input.keys())==2:
        print('-----Start evaluating-----')
        result = rrc_evaluation_funcs.main_evaluation(input, default_evaluation_params, validate_data,evaluate_method)
    elif len(input.keys())==4:
        for key in input.keys():
            if len(eval_true.keys())==2:
                break
            eval_true[key] = input[key]
            eval_fake[key] = input[key+'1']

        print('-----Start evaluating-----')
        print('Evaluating true class:')
        result_true = rrc_evaluation_funcs.main_evaluation(eval_true,default_evaluation_params,validate_data,evaluate_method)
        print('\n')
        print('Evaluating fake class:')
        result_fake = rrc_evaluation_funcs.main_evaluation(eval_fake,default_evaluation_params,validate_data,evaluate_method)
    else:
